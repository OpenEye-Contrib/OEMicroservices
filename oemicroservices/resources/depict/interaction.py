...
# interaction_arg_parser.add_argument('width', type=int, default=800, location='args')    # Image width
# interaction_arg_parser.add_argument('height', type=int, default=600, location='args')   # Image height
# interaction_arg_parser.add_argument('legend', type=bool, default=True, location='args') # Include legend
#
...
def _render_image(receptor, ligand, args):
    ...
    title_location = util.get_title_location(args['title-loc']) # The OpenEye title location (if we have a title)
...
class InteractionDepictor(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            try:
                if 'base64' in payload['ligand'] and payload['ligand']['base64']:
                    ligand_string = base64.b64decode(payload['ligand']['value'].encode('utf-8'))
                else:
                    ligand_string = payload['ligand']['value']
                ligand = OEGraphMol()
                OEReadMolFromBytes(
                    ligand,
                    OEGetFileType(payload['ligand']['format']),
                   False if 'gzip' not in payload['ligand'] else bool(payload['ligand']['gzip']),
                    ligand_string
                )
            ...
            try:
                if 'base64' in payload['receptor'] and payload['receptor']['base64']:
                    receptor_string = base64.b64decode(payload['receptor']['value'].encode('utf-8'))
                else:
                    receptor_string = payload['receptor']['value']
                receptor = OEGraphMol()
                OEReadMolFromBytes(
                    receptor,
                    OEGetFileType(payload['receptor']['format']),
                    False if 'gzip' not in 'receptor' else bool(payload['receptor']['gzip']),
                    receptor_string
                )
            except Exception as ex:
